...
export class TableroService {
  ...
  // Envío no-bloqueante al backend remoto (ver `registrar-partida`), aparte del
  // historial local — solo para el conteo de partidas por streamer del panel.
  private backendRegistrado = false;
  ...
  constructor() {
    ...
    // Apenas se resuelve la carrera REAL (nunca la demo de Pruebas, que usa
    // `primerGanadorDemo` y jamás toca esto), se avisa al backend remoto para
    // el conteo del panel — una sola vez, y sin bloquear ni romper el juego
    // si no hay internet o el backend todavía no está desplegado.
    effect(() => {
      const ganador = this.primerGanador();
      if (!ganador || this.backendRegistrado) return;

      const streamer = this.sesion.nombreUsuario();
      if (!streamer) return; // sin sesión iniciada no hay a quién asociarle el conteo

      const filasJugadas =
        this.jugadores().length > 0 ? this.inscripciones().length / this.jugadores().length : 0;

      fetch('/api/registrar-partida', {
        method: 'POST',
        ...
        // Sin conexión o el backend todavía no está desplegado — no debe
        // romper el juego local, que sigue funcionando igual sin esto.
      });

      this.backendRegistrado = true;
    });
  }
